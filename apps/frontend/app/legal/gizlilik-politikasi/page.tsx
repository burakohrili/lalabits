export const metadata = { title: 'Gizlilik Politikası — lalabits.art' };
...
async function fetchMeta(): Promise<LegalMeta | null> {
  ...
  try {
    const res = await fetch(`${API}/legal/privacy-policy`, { cache: 'no-store' });
  ...
  return (
    <article className="flex flex-col gap-8">
      <header className="flex flex-col gap-2 border-b border-border pb-6">
        <h1 className="text-2xl font-semibold text-foreground">Gizlilik Politikası</h1>
        {meta ? (

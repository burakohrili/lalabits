...
type StatusFilter = 'all' | 'open' | 'under_review' | 'resolved_refund' | 'resolved_no_action' | 'closed';
...
const STATUS_LABELS: Record<string, string> = {
  ...
  resolved_no_action: 'İşlem Yapılmadı',
  closed: 'Kapandı',
};

export default function AdminItirazlarPage() {
  ...
  const fetchDisputes = useCallback(async (status: StatusFilter, p: number) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/billing/admin/disputes?${params.toString()}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      if (res.status === 401) { logout(); return; }
      if (!res.ok) throw new Error('FETCH_FAILED');
      const data = (await res.json()) as { items: DisputeItem[]; total: number };
      setItems(data.items);
      setTotal(data.total);
    } catch {
      setError('İtirazlar yüklenemedi.');
    } finally {
      setLoading(false);
    }
  }, [accessToken, logout]);

  useEffect(() => { void fetchDisputes(statusFilter, page); }, [fetchDisputes, statusFilter, page]);
  ...
  function openPanel(item: DisputeItem) {
  ...
  async function handleUpdate() {
    ...
    setUpdating(true);
    try {
      const res = await fetch(`${API}/billing/admin/disputes/${selectedId}`, {
        method: 'PUT',
      ...
      const updated = (await res.json()) as DisputeItem;
    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-foreground">Ödeme İtirazları</h1>
            <p className="mt-0.5 text-sm text-muted">{total > 0 ? `${total} itiraz` : 'İtiraz yok'}</p>
          </div>
        ...
        <div className="mb-5 flex flex-wrap gap-1 rounded-xl border border-border bg-surface p-1 w-fit">
          {(['all', 'open', 'under_review', 'resolved_refund', 'resolved_no_action', 'closed'] as StatusFilter[]).map((s) => (
            <button
              key={s}
              type="button"
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <div className="rounded-2xl border border-border bg-surface px-6 py-12 text-center">
                <p className="text-sm text-muted">Bu filtreyle eşleşen itiraz yok.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-surface border-b border-border">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-muted">Neden</th>
                  ...
                  <tbody className="divide-y divide-border bg-background">
                    {items.map((item) => (
                      <tr key={item.id} className="hover:bg-surface/50 transition-colors">
                        <td className="px-4 py-3 text-xs text-foreground">{item.fan_email ?? '—'}</td>
                        ...
                        <td className="px-4 py-3">
                          <span className={[
                            ...
                            item.status === 'resolved_refund' ? 'bg-green-50 text-green-700'
                              : item.status === 'open' ? 'bg-amber-50 text-amber-700'
            ...
            <div className="w-72 shrink-0">
              <div className="rounded-2xl border border-border bg-surface p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-semibold text-foreground">İtiraz Güncelle</h2>
